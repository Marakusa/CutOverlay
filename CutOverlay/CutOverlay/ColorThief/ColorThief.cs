...
public class ColorThief
{
    ...
    /// <returns></returns>
    public QuantizedColor GetColor(SKBitmap sourceImage, int quality = DefaultQuality,
    ...
    /// <returns></returns>
    /// <code>true</code>
    public List<QuantizedColor> GetPalette(SKBitmap sourceImage, int colorCount = DefaultColorCount,
    ...
    /// <param name="ignoreWhite">if set to <c>true</c> [ignore white].</param>
    /// <returns></returns>
    private CMap GetColorMap(SKBitmap sourceImage, int colorCount, int quality = DefaultQuality,
        bool ignoreWhite = DefaultIgnoreWhite)
    {
    ...
    private IEnumerable<int> GetIntFromPixel(SKBitmap bmp)
    {
        for (int x = 0; x < bmp.Width; x++)
        for (int y = 0; y < bmp.Height; y++)
        {
            SKColor clr = bmp.GetPixel(x, y);
            yield return clr.Blue;
            yield return clr.Green;
            yield return clr.Red;
            yield return clr.Alpha;
        }
    }

    private int[][] GetPixelsFast(SKBitmap sourceImage, int quality, bool ignoreWhite)
    {
